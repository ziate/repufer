...
@section('title',Request::is('admin/product_review*')?'Products Review List':'Fixed Products List')
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                    <h1 class="page-header-title"> {{ Request::is('admin/product_review*')?'Products Review List':'Fixed Products List' }}<span class="badge badge-soft-dark ml-2" id="Fixed_ProductsCount">{{-- $Fixed_Productss->total() --}}</span></h1>
                </div>
                    ...
                    <div class="table-responsive datatable-custom" id="table-div">
                        <table id="datatable" class="table table-borderless table-thead-bordered table-nowrap table-align-middle card-table"
                            ...
                            @foreach($Fixed_Productss as $key=>$Fixed_Products)
                                @if (Request::is('admin/product_review*'))
                                    @if ($Fixed_Products->status ==0)
                                        <tr>
                                            <td>{{$key+$Fixed_Productss->firstItem()}}</td>
                                            <td>
                                                <a class="media align-items-center" href="{{route('admin.Fixed_Products.view',[$Fixed_Products['id']])}}">
                                                    <img class="avatar avatar-lg mr-3" src="{{asset('storage/app/public/product')}}/{{ json_decode($Fixed_Products['image'])[0]}}"
                                            ...
                                            <td>
                                                ...
                                                <a class="btn btn-sm btn-white" href="javascript:"
                                                    onclick="form_alert('Fixed_Products-{{$Fixed_Products['id']}}','{{__('messages.Want_to_delete_this_item')}}')" title="{{__('messages.delete')}} {{__('messages.Fixed_Products')}}"><i class="tio-delete-outlined"></i>
                                                </a>
                                    ...
                                    @endif
                                @else
                                    @if ($Fixed_Products->status ==1)
                                        <tr>
                                            <td>{{$key+$Fixed_Productss->firstItem()}}</td>
                                            <td>
                                                <a class="media align-items-center" href="{{route('admin.Fixed_Products.view',[$Fixed_Products['id']])}}">
                                                    @if($Fixed_Products['image'] != null)
                                                        <img class="avatar avatar-lg mr-3" src="{{asset('storage/app/public/product')}}/{{ json_decode($Fixed_Products['image'])[0]}}"
                                                             onerror="this.src='{{asset('public/assets/admin/img/160x160/img2.jpg')}}'" alt="{{$Fixed_Products->name}} image">
                                                    @endif
                                                    <div class="media-body">
                        ...
                        <div class="page-area">
                            <table>
                                ...
                                {!! $Fixed_Productss->withQueryString()->links() !!}
                                </tfoot>
                            </table>
                        </div>
                    </div>
...
@push('script_2')
    <script>
        ...
        $('#search-form').on('submit', function (e) {
            ...
            $.post({
                url: '{{route('admin.Fixed_Products.search')}}',
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
                ...
                success: function (data) {
                    ...
                    $('.page-area').hide();
                    $('#Fixed_ProductsCount').html(data.count);
                },
                complete: function () {
                    $('#loading').hide();
                },
