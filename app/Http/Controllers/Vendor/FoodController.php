...
class FoodController extends Controller
{
    public function index()
    {
        ...
        $brands = Brands::all();
        $Fixed_Products = Fixed_Products::where('status',1)->get();
        $categories = Category::where(['position' => 0])->get();
        return view('vendor-views.product.index', compact('categories','Fixed_Products','brands'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'image' => $request['status'] == 0?"":'required',
        ...
        if(isset($request['product_id']) and ($request['status'] == 0)){
            $food = new Food;
        }else{
            $request['status'] = 1;
            $food = new Fixed_Products;
        }
        ...
        $images=[];
        if(isset($request['product_id']) and ($request['status'] == 0)){
            $images = Fixed_Products::where('id',$request['product_id'])->first();
        ...
        $food->status = $request['status']==null?1:0;
    ...
    public function edit($id)
    {
        ...
        return view('vendor-views.product.edit', compact('product', 'product_category', 'categories' ,'brands'));
    }
    ...
    {
        $cat = Sub_Categories::where(['parent_id' => $request->parent_id])->get();
        $res = '<option value="' . 0 . '" disabled selected>---Select---</option>';
        foreach ($cat as $row) {
            if ($row->id == $request->sub_category) {
                $res .= '<option value="' . $row->id . '" selected >' . $row->name . '</option>';
    ...
    public function getDataProductFixed($formData,Request $request){
        $Fixed_Products = Fixed_Products::with('brands')->where('name',$formData)->first();
        return response()->json($Fixed_Products,200);
    }
}
